...
/* Defining real and complex for time domain <--> frequency domain transfer*/
typedef float real;
typedef struct 
{   
    real Re;
...
/* returns a natural number greater-or-equal to the input, where the number returned is a power of 2 */
...
/* returns the magnitude for a complex number */
float  get_magnitude(complex c)
{
...
void ifft(complex *v, int n, complex* tmp, int tag)
{
  if(n>1) {			                        /* otherwise, do nothing and return */
    int k,m;    complex z, w, *vo, *ve;
    ve = tmp; vo = tmp+n/2;
    ...
    if(tag == 0)                            /* so the final series can be scaled down by a factor of n */
      {
        tag = n;
      }
    ifft( ve, n/2, v , 1);		            /* FFT on even-indexed elements of v[] */
    ifft( vo, n/2, v , 1);		            /* FFT on odd-indexed elements of v[] */
    for(m=0; m<n/2; m++) {
      w.Re = cos(2*PI*m/(double)n);
      w.Im = sin(2*PI*m/(double)n);
      z.Re = w.Re*vo[m].Re - w.Im*vo[m].Im;	/* Re(w*vo[m]) */
      z.Im = w.Re*vo[m].Im + w.Im*vo[m].Re;	/* Im(w*vo[m]) */
      v[  m  ].Re = (ve[m].Re + z.Re)/tag;
      v[  m  ].Im = (ve[m].Im + z.Im)/tag;
      v[m+n/2].Re = (ve[m].Re - z.Re)/tag;
      v[m+n/2].Im = (ve[m].Im - z.Im)/tag;
    }
  }
  return;
}
...
/* Does the following:
   -> Input : subjects_HRF     - this is a local variable for each thread which captures the rsHRF time-series for the regions
              h                - this is the length of the HRF time-series
              k                - this points to the current brain region for which we are obtaining the BOLD simulation (local to each thread)
              start            - this points to the starting region for the particular thread (this is only passed so that the function can print out which region is it convolving)
              l                - this is the length of the SIMULATED_signal (time_steps)
              bold_length      - this is the length of the BOLD signal after downsampling (NOTE: this is longer than the required signal as we increased the time_steps to the next power of 2)
              SIMULATED_signal - this is a local variable for each thread which captures the simulated neural response for the regions
  -> Output : An array of float (of length = bold_length) is returned which captures the BOLD response

  -> Workflow : 
    1. complex_signal and complex_hrf_signal are to capture the neural response and hrf respectively, as their complex representations (which has all imaginary values = 0)
    2. The size of all the arrays is (l + h - 1), this is to facilitate the fft required for performing the linear convolution
    3. The values are copied to their respective representations, for the additional places in the arrays 0 padding is applied
    4. Fast Fourier Transformation is used to obtain the Discrete-Fourier-Transform for complex_signal and complex_hrf_signal
    5. The respective dft's are multiplied (element wise) to obtain the dft for the convolved (BOLD) response
    6. Inverse Fast Fourier Transorfmation is used to obtain the time-domain signal of the product of dft's
    7. The real part of the last step's output is copied to the BOLD array. We only copy the real part as both our input signals were real.
    8. The output from last step is downsampled such that the output is in accordance to the BOLD repetition time
    9. The above downsampled output is returned by the function
*/
float* convolve_downsample_subject(float **subjects_HRF, int h, int k, int start, int l,  int bold_length ,float **SIMULATED_signal){

    printf("\nConvolve region %d",start + k+1);

    complex* complex_signal         = (complex *)_mm_malloc((l+h-1) * sizeof(complex), 16);    /* for neural signal in the complex domain */
    complex* complex_hrf_signal     = (complex *)_mm_malloc((l+h-1) * sizeof(complex), 16);    /* for HRF signal in the complex domain */ 

	complex* tmp0                   = (complex *)_mm_malloc((l+h-1) * sizeof(complex), 16);    /* required for the FFT implementation, no physical significance */
    complex* fft_response           = (complex *)_mm_malloc((l+h-1) * sizeof(complex), 16);    /* to store the fft reponse of FFT(Nerual Signal) * FFT(HRF) */

    float* BOLD                     = (float*)_mm_malloc((l+h-1) * sizeof(float), 16);         /* to store the obtained BOLD response */
    float* BOLD_ret                 = (float*)_mm_malloc(bold_length * sizeof(float), 16);     /* return value - downsampled BOLD response */
    
    if(complex_signal == NULL || complex_hrf_signal == NULL || tmp0 == NULL || fft_response == NULL || BOLD == NULL || BOLD_ret == NULL)
    {
        printf( "ERROR: Running out of memory. Aborting... \n");
        exit(2);
    }           
    
    /* defining the complex counter parts of the signals */
    for(int i = 0; i < (l+h-1); i++)
    {
        if(i < l) /* for the duration of the signal length */
            complex_signal[i].Re = SIMULATED_signal[k][i];
        else      /* zero padding */
            complex_signal[i].Re = 0.0;
        complex_signal[i].Im = 0.0;
    }
    
    for(int i = 0; i < (l+h-1); i++)
    {
        if(i < h) /* for the duration of the signal length */
            complex_hrf_signal[i].Re = subjects_HRF[k][i];
        else      /* zero padding */
            complex_hrf_signal[i].Re = 0.0;
        complex_hrf_signal[i].Im = 0.0;
    }
    
    /* Performing the FFT */
    fft(complex_signal, l+h-1, tmp0);
    fft(complex_hrf_signal, l+h-1, tmp0);
    
    /* Element wise multiplication */
    for(int i = 0; i < (l+h-1); i++)
    {
        fft_response[i].Re = (complex_signal[i].Re * complex_hrf_signal[i].Re) - (complex_signal[i].Im * complex_hrf_signal[i].Im);
        fft_response[i].Im = (complex_signal[i].Re * complex_hrf_signal[i].Im) + (complex_signal[i].Im * complex_hrf_signal[i].Re);
    }

    /* Performing the IFFT */
    ifft(fft_response, (l+h-1), tmp0, 0);

    /* Copying the Real part to BOLD */
    for(int i = 0; i < (l+h-1); i++)
        BOLD[i] = (fft_response[i].Re);
      
    /* Downsampling the BOLD response to match BOLD TR */
    resample_sinc(BOLD, (l+h-1), BOLD_ret, (bold_length)); 
    
    /* Freeing Memory */
    _mm_free(tmp0);
    _mm_free(fft_response);
    _mm_free(complex_signal);
    _mm_free(complex_hrf_signal);
    _mm_free(BOLD);
...
void *run_simulation(void *arg)
{
    ...
    int   time_steps          = nextPowerOf2((divRoundClosest(total_duration/model_dt, 1)) + stock_steps - 1);         // Such that the total length of the output signal is a power of 2 (for FFT)
        time_steps            = (time_steps - stock_steps) + 1;                                                        // Extended duration for which we obtain the neural response
        time_steps            = FIC_time_steps + time_steps;                                                           // Total time-steps for the simulation loop
    ...
    int actual_bold_len = divRoundClosest((time_steps - FIC_time_steps)/divRoundClosest(BOLD_TR/model_dt, 1), 1);      // actual length of the BOLD signal (due to time_steps being increased to the next power of 2)
    ...
    {
        SIMULATED_signal[i] = (float *)_mm_malloc((time_steps - FIC_time_steps) * sizeof(float),16);
    ...
    for (ts = 0; ts < time_steps; ts++) 
    {
        ...
        if(ts > FIC_time_steps)
        {
            for (j = 0; j < nodes_mt; j++) 
            {
                SIMULATED_signal[j][ts-FIC_time_steps-1] = S_i_E[j];  
            }     
        }
    ...
    float* BOLD_aux = (float *)_mm_malloc(actual_bold_len * sizeof(float), 16); /* to store the BOLD response output from convolve_downsample_subject function */
    if(BOLD_aux == NULL)
    {
        printf( "ERROR: Running out of memory. Aborting... \n");
        exit(2);
    ...
    for(int k = 0; k < (end_nodes_mt_glob-start_nodes_mt); k++)                                      /* simulating the BOLD response for all the nodes in the thread */
    {
        memset(BOLD_aux, 0, sizeof(BOLD_aux));

        BOLD_aux = convolve_downsample_subject(HRF_signal, stock_steps, k, start_nodes_mt, time_steps-(FIC_time_steps), actual_bold_len,SIMULATED_signal);
        
        for(int i = 0; i < BOLD_TS_len; i++)                               /* Storing the value in the global BOLD_ex (the values only upto the required length is stored) */
            BOLD_ex[((start_nodes_mt + k)*BOLD_TS_len) + i] = BOLD_aux[i];
    }
